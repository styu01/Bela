...
import { toPendingRetryView, classifyTelegramSendError, OWNER_ESCALATION_EXTRA_MS, type PendingRetryView } from '../pending-retries.js'
import {
...
export interface TaskInflightEntry {
  ...
  host: string | null
  injectedAt: number
  // Stage 1: inter-agent notice sent to BÉLA (was, until 2026-08-29, a
  // direct Telegram alert -- see docs/scheduler-escalation-dev-spec.md).
  alerted: boolean
  // Stage 2: direct-to-owner (Telegram) escalation, only after BÉLA has had
  // OWNER_ESCALATION_EXTRA_MS since the stage-1 notice to react. In-memory
...
export type TaskTimeoutDecision = 'clear' | 'alert' | 'escalate' | 'hold'
...
// alert    -- stage 1: one-shot inter-agent notice to BÉLA (session busy past
//             timeout threshold)
// escalate -- stage 2: one-shot direct-to-owner Telegram alert, only once
//             BÉLA has already been notified (alert fired) AND a further
//             OWNER_ESCALATION_EXTRA_MS has elapsed since injection with the
//             session still busy. Keyed off `injectedAt`, not off the
//             stage-1 send succeeding -- see docs/scheduler-escalation-dev-spec.md
//             for why this must not chain on stage 1's success.
...
// that ceiling is an extreme edge case outside this card's scope.
export function decideTaskTimeout(
  entry: Pick<TaskInflightEntry, 'injectedAt' | 'alerted' | 'ownerAlerted'>,
...
// Build the pending-retry alert body shared by stage 1 (BÉLA, inter-agent)
// and stage 2 (owner, Telegram) -- the substance (which task, why stuck,
// how long) is identical, only the delivery channel differs.
...
// Stage 1: inter-agent notice to BÉLA once a pending retry crosses
// ALERT_THRESHOLD_MS. Until 2026-08-29 this went straight to the operator's
// Telegram (see sendPendingRetryAlert below, now the stage-2 escalation) --
// see docs/scheduler-escalation-dev-spec.md for why that was too eager (three
// tasks that all resolved themselves within the hour on 2026-08-29 each fired
// a direct alert for nothing).
function sendPendingRetryBelaNotice(view: PendingRetryView, nowMs: number): void {
  // Stamp first, same claim-before-send guard as the Telegram path: if
  ...
  const text = pendingRetryAlertText(view, '[scheduler]', ageMinutes, firstAttempt) +
    '\nHa nem oldodik meg, Istvan direkt Telegram-ertesitest kap ha ez tovabb tart.'
  try {
    createAgentMessage('system', MAIN_AGENT_ID, text)
    logger.info({ task: view.taskName, agent: view.agentName, ageMinutes }, 'Pending-retry BÉLA notice sent')
  } catch (err) {
    // A failed DB insert here does not block stage-2 escalation -- see
    // sendTaskInflightBelaNotice for the same rationale on the other path.
    logger.warn({ err, task: view.taskName, agent: view.agentName }, 'Pending-retry BÉLA notice failed, clearing stamp for retry')
    clearPendingTaskRetryAlert(view.taskName, view.agentName)
  }
}

// Stage 2: direct-to-owner Telegram alert, only once BÉLA has already been
// notified (stage 1 above) and OWNER_ESCALATION_EXTRA_MS has additionally
// elapsed with the row still pending.
...
// Stage 1: one-shot inter-agent notice to BÉLA when a fired task/heartbeat
// has been continuously busy past TASK_FIRE_TIMEOUT_MS. Until 2026-08-29
// this went straight to the operator's Telegram (see sendTaskTimeoutAlert
// below, now the stage-2 escalation) -- three tasks that all resolved
// themselves within the hour (2026-08-29 08:44 report) each fired a direct
// Telegram alert for nothing. See docs/scheduler-escalation-dev-spec.md.
//
// Moves the matching kanban card to 'waiting' here (stage 1), not in the
// stage-2 escalation -- the board should reflect a stuck task as soon as
// BÉLA is told, independent of whether it later escalates to the operator.
function sendTaskInflightBelaNotice(entry: TaskInflightEntry, elapsedMs: number): void {
  ...
  const text = [
    ...
    'Ellenőrizd az ágenst; ha nem oldódik meg, Istvan direkt Telegram-értesítést kap ha ez tovabb tart.',
  ].join('\n')
  try {
    createAgentMessage('system', MAIN_AGENT_ID, text)
    logger.info({ task: entry.taskName, agent: entry.agentName, ageMinutes }, 'task-timeout BÉLA notice sent')
  } catch (err) {
    ...
    // succeeding), so a broken bela-notice path can never silently
    // suppress the final Telegram escalation.
    logger.warn({ err, task: entry.taskName, agent: entry.agentName }, 'task-timeout BÉLA notice failed')
  }
}

// Stage 2: one-shot direct-to-owner Telegram alert, only once BÉLA has
// already been notified (stage 1 above) and OWNER_ESCALATION_EXTRA_MS has
// additionally elapsed with the session still busy. Follows the same
// token-resolution and ALLOWED_CHAT_ID path as sendPendingRetryAlert: this
...
function sendTaskTimeoutAlert(entry: TaskInflightEntry, elapsedMs: number): void {
  ...
  const thresholdMinutes = Math.round(entry.timeoutMs / 60000)
  const text = [
    `[${BOT_NAME} scheduler] A(z) "${entry.taskName}" (${entry.agentName}) ütemezett feladat ${ageMinutes} perce fut -- lehetséges beakadás. BÉLA mar ertesitve volt, de nem oldodott meg.`,
  ...
  async function runCheck() {
    ...
    for (const [key, entry] of taskInflightMap) {
      ...
      if (decision === 'clear') {
        ...
        sendTaskInflightBelaNotice(entry, now - entry.injectedAt)
        entry.alerted = true
    ...
    for (const row of pendingRows) {
      ...
      if (stillPresent && view.alertDue) sendPendingRetryBelaNotice(view, now)
